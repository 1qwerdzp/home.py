...
while True :
    ...
    for q in range(3,+4) :
        if (int(p1.count(list(str(1+q))))==1 and int(p1.count(list(str(2+q))))==1 and int(p1.count(list(str(3+q))))==1) :
            tx=font.render("1p did win to the game", True, (0, 0, 0))
        elif (int(p2.count(list(str(1+q))))==1 and int(p2.count(list(str(2+q))))==1 and int(p2.count(list(str(3+q))))==1) :
            tx=font.render("2p did win to the game", True, (0, 0, 0))
    for t in range(0,+3) :
        if (int(p1.count(list(str(1+t))))==1 and int(p1.count(list(str(5))))==1 and int(p1.count(list(str(7+t))))==1) :
            tx=font.render("1p did win to the game", True, (0, 0, 0))
        elif (int(p2.count(list(str(1+t))))==1 and int(p2.count(list(str(5))))==1 and int(p2.count(list(str(5))))==1) :
            tx=font.render("2p did win to the game", True, (0, 0, 0))
    if len(pa)==10 :
        tx=font.render("the game is not over", True, (0, 0, 0))
        SC.blit(tx, (0,0))
